...
function local_random_avatar( $avatar, $id_or_email, $size, $default, $alt) {
    ...
    $list_Size = 15;
    $root_Hash = 'f3bb4e1b734cbcd095dadbe7ecb53aaa'; // 博主ID的hash

    if($email_hash == $root_Hash) { // 为博主自定义高清头像
        $avatar = "<img alt='{$alt}' src='//amagi.yukisaki.io/wp-content/uploads/2020/02/IMG_3793.jpeg' class='avatar avatar-{$size} photo' height='{$size}' width='{$size}' />"; 
        return $avatar;
    } else {
        $random = crc32($email_hash) % $list_Size + 1;
        $src = ''.get_template_directory_uri().'/images/avatars/'. $random .'.jpg';
